...
The GitHub-only `daily` branch adds the latest `r-next` and `r-devel` snapshots
to the historical released versions. Install it with:

```r
devtools::install_github("hughjonesd/rcheology", ref = "daily")
```

The `rcheology$status` column distinguishes `"released"`, `"r-next"`, and
`"r-devel"` rows. The latest source package and data files are also available
from the [daily GitHub release](https://github.com/hughjonesd/rcheology/releases/tag/daily-release).
...
* Run `host-run-on-evercran.sh` to collect functions and help in one pass.
...
```{r}
old_mean_help <- version_help("mean", "1.9.1", package = "base", format = "text")
